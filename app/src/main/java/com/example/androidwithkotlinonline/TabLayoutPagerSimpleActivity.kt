...
class ViewPagerAdapter(
    ...
    override fun isViewFromObject(view: View, `object`: Any): Boolean {
        ...
        // === -> 주소값을 비교한다
        // a = 1, b = 1 -> a == b (true)
        // a === b (true, false)  변수의 값은 같아도 주소값은 같을수도 다를수도 있다
    }
    ...
    override fun instantiateItem(container: ViewGroup, position: Int): Any {
        when (position) {
            0 -> {
                val view = layoutInflater.inflate(R.layout.fragment_01, container, false)
                container.addView(view)
                return view
            }
            1 -> {
                val view = layoutInflater.inflate(R.layout.activity_phonebook_itemlist, container, false)
                container.addView(view)
                return view
            }
            else -> {
